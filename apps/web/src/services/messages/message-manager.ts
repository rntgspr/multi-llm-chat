 ...
 */
export function getHiddenMessagesForAssistant(roomId: RoomId, assistantId: AssistantId): Message[] {
  const messages = messagesByRoom.get(roomId) || []

  return messages.filter(
    (m) => m.visibility === 'hidden' && (m.recipientId === assistantId || m.recipientId === undefined)
  )
}
 ...
 */
export function prepareContextForAssistant(roomId: RoomId, assistantId: AssistantId, contextLimit = 50): Message[] {
