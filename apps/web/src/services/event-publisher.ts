...
import { getPublisher } from "@multi-llm/maintenance";
import type {
  MessageSentPayload,
 ...
 */
export async function publishUserStatusChanged(
  userId: string,
  ...
  await publisher.publishEvent<UserStatusChangedPayload>(
    "user.status.changed",
    "status.changed",
    payload,
  );
 ...
 */
export async function publishRoomMemberJoined(
  roomId: string,
  userId: string,
): Promise<void> {
  const publisher = getPublisher("web");
...
export async function publishBatchEvents(
  ...
  const promises = events.map((event) =>
    publisher.publishEvent(event.channel, event.type, event.payload),
  );
