 ...
 */
export async function cacheUserPreferences(userId: string, preferences: Record<string, unknown>): Promise<void> {
 ...
 */
export async function getCachedUserPreferences(userId: string): Promise<Record<string, unknown> | null> {
  const cache = getCacheClient()

  const preferences = await cache.get<Record<string, unknown>>(`user:${userId}:preferences`)
 ...
 */
export async function cacheApiResponse(endpoint: string, data: unknown, ttlSeconds: number = 300): Promise<void> {
  const cache = getCacheClient()
...
export async function checkCacheHealth(): Promise<boolean> {
  ...
  console.log(`Cache health: ${health.ok ? 'OK' : 'FAILED'} (latency: ${health.latencyMs}ms)`)
