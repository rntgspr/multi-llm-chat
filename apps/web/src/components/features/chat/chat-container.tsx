  ...
  if (error || !data?.room) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-background">
        ...
        <Link href="/rooms" className="flex items-center gap-2 text-primary hover:underline">
    ...
    <div className="flex h-screen bg-background">
      <div className="flex flex-1 flex-col min-w-0">
        <header className="flex items-center gap-3 border-b border-border px-4 py-3 bg-background/80 backdrop-blur-sm sticky top-0 z-10">
          <Link href="/rooms" className="p-1.5 rounded-lg hover:bg-muted transition-colors">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className={`h-2 w-2 rounded-full ${connected ? 'bg-primary' : 'bg-yellow-500'}`} />
