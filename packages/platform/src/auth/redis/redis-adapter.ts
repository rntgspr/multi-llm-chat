...
export function RedisAdapter(): Adapter {
  ...
  return {
    ...
    async createSession(session: { sessionToken: string; userId: string; expires: Date }): Promise<AdapterSession> {
      const ttlSeconds = Math.floor((session.expires.getTime() - Date.now()) / 1000)
    ...
    async updateSession(
      ...
      const ttlSeconds = session.expires ? Math.floor((session.expires.getTime() - Date.now()) / 1000) : undefined

      const updated = await sessionStore.update(session.sessionToken, {}, ttlSeconds)
