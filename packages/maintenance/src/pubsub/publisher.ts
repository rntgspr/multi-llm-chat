...
import type { PubSubEnvelope, PubSubPublisher } from "./types.js";
...
export class RedisPubSubPublisher implements PubSubPublisher {
  ...
  async publish<TPayload>(
    channel: string,
    envelope: PubSubEnvelope<TPayload>,
  ): Promise<void> {
    try {
      ...
      console.log(
        `[Publisher] Published to ${channel} (${subscriberCount} subscribers) - ${fullEnvelope.messageId}`,
      );
    } catch (error) {
