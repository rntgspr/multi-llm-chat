...
import { loadRedisConfig } from "../cache/config.js";
import type {
  PubSubEnvelope,
  PubSubSubscriber,
  Unsubscribe,
...
export class RedisPubSubSubscriber implements PubSubSubscriber {
  ...
  private readonly handlers: Map<
    string,
    Array<(message: PubSubEnvelope<any>) => void | Promise<void>>
  > = new Map();
  ...
  async subscribe<TPayload>(
    ...
    if (handlers) {
      ...
      console.log(
        `[Subscriber] Added handler to ${channel} (${handlers.length} total handlers)`,
      );
    }
  ...
  private async unsubscribe<TPayload>(
    ...
    if (index !== -1) {
      ...
      console.log(
        `[Subscriber] Removed handler from ${channel} (${handlers.length} remaining)`,
      );
    }
  ...
  private handleMessage(channel: string, message: string): void {
    ...
    try {
      ...
      console.log(
        `[Subscriber] Received message on ${channel}: ${envelope.messageId}`,
      );
      ...
      for (const handler of handlers) {
        try {
          ...
          if (result instanceof Promise) {
            result.catch((error) => {
              console.error(
                `[Subscriber] Handler error for ${channel}:`,
                error,
              );
            });
          }
      ...
      console.error(
        `[Subscriber] Failed to parse message from ${channel}:`,
        error,
      );
