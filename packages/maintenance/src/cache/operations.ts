...
import { getRedisClient, healthCheck as redisHealthCheck } from "./redis-client.js";
import type { CacheClient, CacheSetOptions, CacheError } from "./types.js";
  ...
  async set<T>(
    ...
    try {
      ...
      if (options?.ttlSeconds) {
        ...
        console.log(
          `[CacheClient] Set key ${key} with TTL ${options.ttlSeconds}s`,
        );
      } else {
      ...
      if (
        (error as CacheError).code === "SERIALIZATION_ERROR"
      ) {
        throw error;
      }
