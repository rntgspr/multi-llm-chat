...
**Aplica-se a:** `**/*.{ts,tsx,js,jsx}`

## Idioma

- **Inglês**: comentários JSDoc, identificadores de código (variáveis, funções, tipos, nomes de arquivo) e textos de UI.
- **Português**: todo conteúdo em markdown dentro de `.github/` e `openspec/` (instruções, prompts, agentes, specs, plans, constitution) e a conversa com o assistente.
