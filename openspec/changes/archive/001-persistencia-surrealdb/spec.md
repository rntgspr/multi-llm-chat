...
O desenvolvedor do projeto precisa que todo o código de acesso ao banco de dados esteja centralizado no pacote `@synergy/db`, sem duplicações em `apps/api`. Atualmente existem cópias idênticas dos repositories e do cliente SurrealDB em `apps/api/src/repositories/` e `apps/api/src/lib/db*`. Essas duplicações devem ser removidas e `apps/api` deve consumir `@synergy/db` como dependência de workspace.
...
1. **Given** os repositories duplicados em `apps/api/src/repositories/`, **When** o desenvolvedor remove esses arquivos e adiciona `@synergy/db` como dependência, **Then** todos os imports em `apps/api/src/` apontam para `@synergy/db` e `pnpm typecheck` passa sem erros.
...
3. **Given** `apps/api/package.json` sem a dependência `@synergy/db`, **When** o desenvolvedor adiciona `"@synergy/db": "workspace:*"` às dependências, **Then** `pnpm install` resolve corretamente a dependência e os módulos são acessíveis.
...
As interfaces públicas dos modelos de banco de dados e dos repositories devem ser documentadas como contratos do projeto, servindo como referência autoritativa para consumidores do pacote `@synergy/db`.
...
**Independent Test**: Verificar que os contratos refletem fielmente as interfaces exportadas por `@synergy/types/db` e `@synergy/db`.
...
- **FR-002**: O sistema DEVE manter todo o código de acesso ao banco centralizado no pacote `@synergy/db`, sem duplicações em outros apps do monorepo.
...
- **SC-005**: Nenhum código de acesso ao banco existe fora do pacote `@synergy/db` — centralização completa.
...
- A rede Docker `synergy-network` é compartilhada entre os compose files de infra e LLMs.
