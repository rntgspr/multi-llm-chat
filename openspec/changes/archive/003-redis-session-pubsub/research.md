...
## Decision 2: Centralize Redis runtime logic in `@synergy/maintenance`

- **Decision**: Implement Redis session/cache/pubsub runtime abstractions in `packages/maintenance`.
- **Rationale**: Both `apps/web` and `apps/api` already depend on `@synergy/maintenance`; this avoids duplicate clients and keeps auth/cache lifecycle concerns in a single bounded context.
- **Alternatives considered**:
